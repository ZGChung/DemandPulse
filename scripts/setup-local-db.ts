...
async function setupLocalDatabase() {
  ...
  const hasDatabaseUrl = envContent.includes('\nDATABASE_URL=') &&
                        !envContent.includes('\n# DATABASE_URL=');
  ...
  try {
    ...
    console.log('\n   Or use Docker:');
    console.log('   docker run --name demandpulse-postgres -e POSTGRES_PASSWORD=password -d -p 5432:5432 postgres:16-alpine');
    return;
  }
