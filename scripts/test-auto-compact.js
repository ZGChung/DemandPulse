...
async function simulateConversation() {
  ...
  try {
    ...
    const testMessages = [
      {
        role: 'user',
        content: 'I need to build a new feature for my application. Can you help me create a user authentication system?'
      },
      {
        role: 'assistant', 
        content: 'Sure! I can help you build a user authentication system. What stack are you using?'
      },
      {
        role: 'user',
        content: 'I\'m using Next.js with Prisma and PostgreSQL. I need JWT-based authentication with email/password login and social login options.'
      },
      {
        role: 'assistant',
        content: 'Great choice! Let me create a comprehensive authentication system for you. First, I\'ll set up the database schema...'
      },
    ...
    for (const message of testMessages) {
      ...
      console.log(`   Context: ${contextStatus.messageCount} messages, ${Math.round(contextStatus.usagePercentage * 100)}% usage`)
    ...
    history.forEach((event, i) => {
      console.log(`   ${i + 1}. ${event.timestamp.toISOString()} - ${event.strategy} - ${event.status}`)
    })
