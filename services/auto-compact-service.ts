...
export class AutoCompactService {
  ...
  private async handleAutoCompactTrigger(data: any): Promise<void> {
    ...
    try {
      ...
      this.notify(`Auto-compact ${result.success ? 'completed' : 'failed'}: ${result.message}`, 
  ...
  private async executeCompact(strategy: string): Promise<{
    ...
    console.log(`Executing compact with strategy: ${strategy}, method: ${this.config.executionMethod}`)
  ...
  private async executeCompactCLI(strategy: string): Promise<{
    ...
    console.log(`Would execute CLI: ${this.config.cliPath} ${this.config.compactCommand} --strategy=${strategy}`)
  ...
  private notify(message: string, type: 'info' | 'warning' | 'error' | 'success', data?: any): void {
    ...
    if (this.config.notificationMethod === 'notification' || this.config.notificationMethod === 'both') {
  ...
  public getStatistics(): {
    ...
    const lastCompact = this.compactHistory.length > 0 
