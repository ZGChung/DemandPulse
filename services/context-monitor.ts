...
export class ContextMonitorService {
  ...
  private extractMessageMetadata(text: string): ConversationMessage['metadata'] {
    return {
      containsRequirements: /\b(requirement|need|want|wish|should have|would be great)\b/i.test(text),
      containsCode: /```[\s\S]*?```|function\s+\w+|class\s+\w+|const\s+\w+|let\s+\w+|var\s+\w+/.test(text),
      containsInstructions: /\b(do|make|create|build|implement|fix|add|remove|change)\b.*\b(please|can you|could you)\b/i.test(text)
    }
  }
  ...
  public getContextStatus(): ContextStatus {
    ...
    if (importantMessages > this.config.maxPreservedMessages) {
      recommendations.push(`Many important messages (${importantMessages}). Consider manual review before compacting.`)
    }
  ...
  private summarizeOldestMessages(): void {
    ...
    for (let i = 0; i < cutoffIndex; i++) {
      this.conversation[i].content = `[Summarized: ${this.conversation[i].content.substring(0, 100)}...]`
