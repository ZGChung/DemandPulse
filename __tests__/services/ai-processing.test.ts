...
describe('AIProcessingService', () => {
  ...
  describe('generateSummary', () => {
    it('should generate summary successfully', async () => {
      ...
      const result = await service.generateSummary('I want to build a dashboard for data visualization')
    ...
    it('should return truncated text on error', async () => {
      ...
      const longText = 'Build a comprehensive system that handles user authentication, data storage, real-time notifications, and reporting with beautiful dashboards and mobile support'
    ...
    it('should cluster requirements by category', async () => {
      ...
      ;(fetch as jest.Mock)
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({ 
            choices: [{ message: { content: JSON.stringify({ categories: ['authentication'], confidence: 0.8 }) } }] 
          }),
        })
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({ 
            choices: [{ message: { content: JSON.stringify({ categories: ['authentication'], confidence: 0.9 }) } }] 
          }),
        })
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({ 
            choices: [{ message: { content: JSON.stringify({ categories: ['database'], confidence: 0.7 }) } }] 
          }),
        })
